...
def advise():
    ...
    return render_template("conclusion.html",t=sum,adv=adv,lim=limit,de=consult_de)

@app.route('/logout')
def logout():
    session['name'] = session['bmi'] = session['bmr'] = session['tdee'] = session['tdee_food'] = session['limit_cal'] = session['bmi_status'] = session['callimit'] = session['forlist'] = session['sumcal'] = session['foods'] = session['foodscal'] = session['consult'] = session['advice'] = None
    return redirect('/')
